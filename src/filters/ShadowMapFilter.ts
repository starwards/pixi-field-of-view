...
export class ShadowMapFilter extends Filter {
    ...
    constructor(private pointOfView: PointOfView) {
        ...
        this.uniforms.dimensions = [pointOfView.radialResolution, pointOfView.pointCount];
    }
    ...
    apply(filterManager: FilterSystem, input: RenderTexture, output: RenderTexture) {
        ...
        const sc = this.pointOfView._shadowCasterSprite;
        ...
        this.uniforms.depthResolution = range * this.pointOfView.depthResolution;

        // Check if there is an object that the filter should attempt to ignore
        const isc = this.pointOfView.ignoreShadowCaster;
